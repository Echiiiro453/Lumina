    ...
    for (let ch = 0; ch < chs; ch++) {
      ...
      for (let i = 0; i < inCh.length; i++) {
        ...
        if (this.mode === 'tape') {
          ...
          // Domain updates (asymmetric hysteretic rates)
          const rate1 = 0.30 * (1.0 + 0.5 * Math.sign(deltaH) * Math.tanh(M1 / 0.5));
          const nextM1 = M1 + rate1 * (Math.tanh(H_eff1 * 2.0) - M1);
          const dM1 = nextM1 - M1;
          M1 = nextM1;
          this.magState1[ch] = M1;

          const rate2 = 0.20 * (1.0 + 0.4 * Math.sign(deltaH) * Math.tanh(M2 / 0.5));
          const nextM2 = M2 + rate2 * (Math.tanh(H_eff2 * 1.3) - M2);
          const dM2 = nextM2 - M2;
          M2 = nextM2;
          this.magState2[ch] = M2;

          const rate3 = 0.12 * (1.0 + 0.3 * Math.sign(deltaH) * Math.tanh(M3 / 0.5));
          const nextM3 = M3 + rate3 * (Math.tanh(H_eff3 * 0.9) - M3);
          const dM3 = nextM3 - M3;
          M3 = nextM3;
          this.magState3[ch] = M3;

          const rate4 = 0.07 * (1.0 + 0.25 * Math.sign(deltaH) * Math.tanh(M4 / 0.5));
          const nextM4 = M4 + rate4 * (Math.tanh(H_eff4 * 0.5) - M4);
          const dM4 = nextM4 - M4;
          M4 = nextM4;
          this.magState4[ch] = M4;

          const rate5 = 0.03 * (1.0 + 0.15 * Math.sign(deltaH) * Math.tanh(M5 / 0.5));
          ...
          const tapeNoise = lfN * lfNoiseLevel + hfN * hfNoiseLevel + barkhausenNoise;
          xFeed = M + tapeNoise;
        }
